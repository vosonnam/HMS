...
function showData (data) {
  ...
  for (row in rows){
    ...
    html+='<td >';
    html+='  <a class="btn btn-transparent btn-xs tooltips btn_read" tooltip-placement="top" tooltip="Remove"><i class="fa fa-file"></i></a>';
    html+='</td>';
    html+='</tr>';
  }
  $(".tb_data").html(html);
...
$(".tb_data").on('click', '.btn_read', function(e){
  e.preventDefault();
  var row=$(this).parents('tr')[0];
  var cell=$('>td',row)[0].innerText;
  sessionStorage.setItem("contactus",cell);
